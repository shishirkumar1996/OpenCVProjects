...
class projectTen(QDialog):
    ...
    def setColor1(self):
        self.color1_lower = np.array([self.hMin.value(),
                                      self.sMin.value(),
                                      self.vMin.value()],
                                     np.uint8)
        self.color1_upper = np.array([self.hMax.value(),
                                      self.sMax.value(),
                                      self.vMax.value()],
                                     np.uint8)
        self.color1_Label.setText('Min: '+str(self.color1_lower)
                                              +' Max :'+
                                              str(self.color1_upper ))
    ...
    def update_frame(self):
        ...
        color_lower = np.array([self.hMin.value(),self.sMin.value()
                                ,self.vMin.value()],np.uint8)

        color_upper = np.array([self.hMax.value(),
                                self.sMax.value(),
                                self.vMax.value()],np.uint8)
    ...
    def track_colored_object(self,img):
        ...
        color_mask = cv2.inRange(hsv,self.color1_lower,
                                 self.color1_upper)
        ...
        maskOpen = cv2.morphologyEx(dilate,cv2.MORPH_OPEN
                                    ,kernelOpen)
        maskClose = cv2.morphologyEx(maskOpen,cv2.MORPH_CLOSE,
                                     kernelClose)

        (_,contours,heirarchy)=cv2.findContours(maskClose,
                                                cv2.RETR_EXTERNAL,
        ...
        for cnt in contours:
            ...
            if (area>5000):
                ...
                cv2.putText(img,'Object Detected',(x,y),cv2.FONT_HERSHEY_SIMPLEX
                            ,0.8,(0,255,0),2)
